...
export default function SeeyonCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (!ticket) {
      location.replace(`${appPrefix}/admin/signin?error=missing_ticket`);
      return;
    }
    ...
    api
      ...
      .then(async (res) => {
        ...
        const target = customPath
          ? `${appPrefix}/admin/${customPath}?token=${encodeURIComponent(token)}`
          : `${appPrefix}/admin/?token=${encodeURIComponent(token)}`;
  ...
  const home = () => location.replace(`${appPrefix}/admin/`);
